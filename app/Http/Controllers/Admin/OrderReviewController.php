...
class OrderReviewController extends Controller {
    ...
    public function audit(Request $request) {
        ...
        if (empty($ids) || empty($storeId) || empty($auditStatus) || !is_array($ids)) {//empty($reviewer) ||
            return Response::json([], -1, Constant::PARAMETER_STRING_DEFAULT);
        }
        $data = OrderReviewService::audit($storeId, $ids, $auditStatus, $reviewer);
     ...
     * @return \Illuminate\Http\JsonResponse
     */
    public function auditRemarks(Request $request) {

        $data = OrderReviewService::auditRemarks($request->all());
