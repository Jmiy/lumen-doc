...
class ActivityHelpedLogService extends BaseService {
            ...
            try {
                ...
                try {
                    ...
                    if ($actctivityApplyData) {//如果已经解锁成功，就更新解锁状态，发送解锁成功邮件
                        $countryJudgeConfig = ActivityService::getActivityConfigData($storeId, $actId, 'apply', 'stock_deduct');
                        $isStockDeduct = data_get($countryJudgeConfig, "apply_stock_deduct.value");
                        if ($isStockDeduct) {
                            //var_dump($isStockDeduct); //注释调试代码 邀请注册活动-优化获取IP组件（针对GB转化为UK）Jmiy 2021-05-05 14:50  update : http://jira.patozon.net/browse/GW-1052
                            //库存扣减新逻辑,改动原因，litom官网邀请活动修改
                            //var_dump([$storeId, $applyId, $productId]);//注释调试代码 邀请注册活动-优化获取IP组件（针对GB转化为UK）Jmiy 2021-05-05 14:50  update : http://jira.patozon.net/browse/GW-1052
                            static::stockDeduct($storeId, $applyId, $productId);
                        } else {
                            //库存扣减现有逻辑
     ...
     * @throws \Exception
     */
    public static function stockDeduct($storeId, $applyId, $productId) {
        $nowTime = Carbon::now()->toDateTimeString();
        //更新库存
        $updateData = [
            Constant::DB_TABLE_QTY_APPLY => DB::raw('qty_apply+1'),
        ...
        $productItem = ActivityProductItemService::getModel($storeId)
            ->select(Constant::DB_TABLE_PRIMARY)
            ->where(Constant::DB_TABLE_PRODUCT_ID, $productId)
            ->first();
        $productItemId = data_get($productItem, Constant::DB_TABLE_PRIMARY);

        $product = ActivityProductService::getModel($storeId)
            ->select('help_sum')
            ->where(Constant::DB_TABLE_PRIMARY, $productId)
            ->first();
        $helpSum = data_get($product, 'help_sum');
        ...
        $isUpdated = ActivityApplyService::update($storeId, $activityApplyWhere, $updateData);
        if (empty($isUpdated)) {//如果更新解锁状态失败，回滚所有的更新操作
            //更新解锁状态失败
            throw new \Exception(null, 61110);
        }
