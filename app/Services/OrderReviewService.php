...
class OrderReviewService extends BaseService {
    ...
    public static $customerInfoName = 'ci';
    ...
    public static function getListData($params, $toArray = true, $isPage = true, $select = [], $isRaw = false, $isGetQuery = false, $isOnlyGetCount = false)
    {
        ...
        if (data_get($params, Constant::DB_EXECUTION_PLAN_IS_ONLY_GET_PRIMARY, false)) {//如果仅仅获取主键id，就不需要处理数据，不关联
            ...
            $auditStatusData = static::$auditMap; //审核状态 -1:未提交审核 0:未审核 1:审核通过 2:审核不通过 3:其他 4:二次催评 5:已返现
            ...
            $handleData = [
                ...
                Constant::DB_TABLE_TYPE.'_show' => FunctionHelper::getExePlanHandleData(Constant::DB_TABLE_TYPE, data_get(static::$rewardType, '-1', ''), static::$rewardType, $dataType, $dateFormat, $time, $glue, $isAllowEmpty, $callback, $only), //礼品类型: 0:其他 1:礼品卡 2:coupon 3:实物 5:积分
            ];
            ...
            $itemHandleDataCallback = [
                Constant::AUDIT_STATUS => function ($item) {//审核状态
                    return (data_get($item, Constant::DB_TABLE_CONTACT_US_ID) > 0 && data_get($item, Constant::AUDIT_STATUS) == '未提交') ? '已发售后邮件' : data_get($item, Constant::AUDIT_STATUS);
                },
                Constant::DB_TABLE_REVIEW_TIME => function ($item) {//审核时间
                ...
                Constant::DB_TABLE_QUANTITY => function ($item) {//订单总产品数
                ...
                Constant::DB_TABLE_SKU => function ($item) {//订单item sku
                ...
                Constant::DB_TABLE_ASIN => function ($item) {//订单item asin
                ...
                'title' => function ($item) {//订单item title
                    return data_get($item, 'items.0.title', Constant::PARAMETER_STRING_DEFAULT);
                },
                Constant::SHOP_NAME => function ($item) {//订单item 店铺名称
                    return data_get($item, 'items.0.' . Constant::SHOP_NAME, Constant::PARAMETER_STRING_DEFAULT) . data_get($item, Constant::DB_TABLE_COUNTRY, Constant::PARAMETER_STRING_DEFAULT);
                },
                'product_quantity' => function ($item) {//订单item 产品数量
                    return data_get($item, 'items.0.quantity', Constant::PARAMETER_STRING_DEFAULT);
                },
                'three_category_name' => function ($item) {//产品三级类目名称
                    $three_category_name = data_get($item, 'items.0.product_category.three_category_name');
                    $two_category_name = data_get($item, 'items.0.product_category.two_category_name');
                    $one_category_name = data_get($item, 'items.0.product_category.one_category_name');

                    return $three_category_name ? $three_category_name : ($two_category_name ? $two_category_name : $one_category_name);
                },
                Constant::DB_TABLE_TYPE_VALUE => function ($item) use ($storeId) {//礼品值
                    //礼品类型: 0:其他 1:礼品卡 2:coupon 3:实物 5:积分

                    $rewardName = data_get($item, Constant::DB_TABLE_REWARD_NAME, Constant::PARAMETER_STRING_DEFAULT);
                    $rewardTypeValue = data_get($item, Constant::DB_TABLE_TYPE_VALUE, Constant::PARAMETER_STRING_DEFAULT);
                    $type = data_get($item, Constant::DB_TABLE_TYPE);

                    if (!in_array($type, [0, 1, 5])) {//如果不是 0:其他 1:礼品卡 5:积分，就直接返回空
                        return Constant::PARAMETER_STRING_DEFAULT;
                    }

                    if (in_array($type, [0, 5])) {//如果是 0:其他 5:积分，就直接返回类型值
                        return $rewardTypeValue;
                    }

                    //如果是 1:礼品卡，就从礼品值中获取礼品卡金额
                    $rewardTypeValue = RewardService::getClientRewardTypeValue($storeId, $rewardName, $type, $rewardTypeValue);
                    return $rewardTypeValue == Constant::PARAMETER_INT_DEFAULT ? Constant::PARAMETER_STRING_DEFAULT : $rewardTypeValue;
                },
                'full_name' => function ($item) {//完整名称

                    $firstName = data_get($item, 'customer_info' . Constant::LINKER . Constant::DB_TABLE_FIRST_NAME, Constant::PARAMETER_STRING_DEFAULT);
                    $lastName = data_get($item, 'customer_info' . Constant::LINKER . Constant::DB_TABLE_LAST_NAME, Constant::PARAMETER_STRING_DEFAULT);
            ...
            $statusCustomizeWhere = [
                [
                    ...
                    Constant::PARAMETERS_KEY => [function ($query) use ($auditStatus) {
                        if (in_array(3, $auditStatus)) $auditStatus[] = 4; //筛选其他类型的状态时，加上索要Rv审核状态
                        if (is_array($auditStatus)) {
...
//            ];
     ...
     * @param string $systemRemarks 系统备注（202103051000 会员系统延保系统管理后台优化1.0 把审核备注改成系统备注 修改人：Jmiy）
     * @param boolean $isAutoAudit 是否自动审核  true:是  false:否  默认：false
     * @return array
     */
    public static function audit($storeId, $ids, $auditStatus, $reviewer, $systemRemarks = null, $isAutoAudit = false)
    {
        ...
        if (null !== $systemRemarks) {
            $updateData[Constant::SYSTEM_REMARKS] = $systemRemarks;//系统备注
        }
    ...
    public static function handleReviewEmail($storeId, $reviewId = 0, $auditStatus = 1, $review = [])
    {
        ...
        if ($storeId == 1 && in_array($type, [0, 1]) && $auditStatus == 1) $toEmail = 'warranty@xmpow.com'; //mpow,审核通过,礼品卡/其他奖励类型邮件发送到指定邮箱
        if ($storeId == 6 && in_array($type, [0, 1]) && $auditStatus == 1) $toEmail = 'Support@homasy.com'; //homasy,审核通过,礼品卡/其他奖励类型邮件发送到指定邮箱
        if ($storeId == 12 && in_array($type, [0, 1]) && $auditStatus == 1) $toEmail = 'warranty_amz@mpow.co.jp'; //mpowJapan,审核通过,礼品卡/其他奖励类型邮件发送到指定邮箱
    ...
    public static function getEmailData($storeId, $auditStatus, $review)
    {
        ...
        if (!empty($langStore)) {
            $emailCountry = strtoupper($langStore);
        }
        ...
        switch ($auditStatus) {
            case 1://审核通过
                //折扣码或者积分奖励或者mpow实物
                if ($type == 2 || $type == 5 || (in_array($type, [3]) && in_array($storeId, OrderWarrantyService::getWarranyStore($storeId)))) {
        ...
        if (in_array($storeId, OrderWarrantyService::getWarranyStore($storeId, 'v2_email_subject'))) {
        ...
        if (in_array($storeId, OrderWarrantyService::getWarranyStore($storeId, 'v2_email_from_name'))) {//mpow和vt和homasy和litom和atmoko  直接使用账户即可
        ...
        //处理礼品面值
        if (in_array($storeId, OrderWarrantyService::getWarranyStore($storeId)) && in_array($type, [0, 1, 2])) { //mpow,vt  礼品类型: 0:其他 1:礼品卡 2:coupon

            //礼品值
            $rewardName = data_get($review, Constant::DB_TABLE_REWARD_NAME, Constant::PARAMETER_STRING_DEFAULT);//奖励名称
            $rewardTypeValue = data_get($review, Constant::DB_TABLE_TYPE_VALUE, Constant::PARAMETER_STRING_DEFAULT);//奖励类型值
            $currency = '';//订单货币

            //礼品卡
            if (in_array($type, [1])) {//如果是礼品卡，就获取订单货币
                $order = OrderService::existsOrFirst($storeId, '', [Constant::DB_TABLE_UNIQUE_ID => data_get($review, Constant::DB_TABLE_ORDER_UNIQUE_ID)], true, [Constant::DB_TABLE_PRESENTMENT_CURRENCY]);
                $currency = data_get($order, Constant::DB_TABLE_PRESENTMENT_CURRENCY, Constant::PARAMETER_STRING_DEFAULT);
            }

            $value = RewardService::getClientRewardTypeValue($storeId, $rewardName, $type, $rewardTypeValue, $currency);

            $key = '*|GIFT_VALUE|*';
            switch (true) {
                case (in_array($type, [2]))://coupon
                    $key = '*|COUPON_VALUE|*';
                    break;
        ...
        if ($auditStatus == 6 && in_array($storeId, OrderWarrantyService::getWarranyStore($storeId))) { //系统催评
            $where = [
                Constant::DB_TABLE_STORE_ID => $storeId,
                Constant::DB_TABLE_TYPE => 'audit_email',
                'conf_key' => [
...
//            data_set($params, 'review_end_time', $curDate);
    ...
    public static function getReviewList($params, $isPage = true, $select = [], $isGetQuery = false, $isOnlyGetCount = false)
    {
        ...
        $itemHandleDataCallback = [
            ...
            Constant::DB_TABLE_ORDER_TIME => function ($item) {//下单时间
    ...
    public static function getSelect()
    {
        return [
            ...
            static::$orderReviewName . Constant::LINKER . Constant::DB_TABLE_CUSTOMER_PRIMARY,
            static::$orderReviewName . Constant::LINKER . Constant::DB_TABLE_REWARD_NAME,
            static::$orderReviewName . Constant::LINKER . Constant::DB_TABLE_TYPE_VALUE,
        ];
    }
...
//                    }
    ...
    public static function handleReviews($storeId, $itemData)
    {
        ...
        $currency = data_get($order, Constant::DB_TABLE_PRESENTMENT_CURRENCY, Constant::PARAMETER_STRING_DEFAULT);//订单货币
        ...
        foreach ($reviews as $review) {
            ...
            data_set($reviewsTmp, $key, $review);
            data_set($reviewsTmp, $key . Constant::LINKER . 'title', Constant::PARAMETER_STRING_DEFAULT);
            data_set($reviewsTmp, $key . Constant::LINKER . 'img', Constant::PARAMETER_STRING_DEFAULT);
        }
        ...
        foreach ($items as $item) {
            ...
            $key = md5("{$sku}_{$asin}");
            if (!isset($reviewsTmp[$key])) {
                continue;
            }

            data_set($reviewsTmp, $key . Constant::LINKER . 'title', data_get($item, 'title', Constant::PARAMETER_STRING_DEFAULT));
            data_set($reviewsTmp, $key . Constant::LINKER . 'img', data_get($item, 'img', Constant::PARAMETER_STRING_DEFAULT));
            data_set($reviewsTmp, $key . Constant::LINKER . 'reward_value', Constant::PARAMETER_STRING_DEFAULT);

            $type = data_get($reviewsTmp, $key . Constant::LINKER . Constant::DB_TABLE_TYPE, Constant::PARAMETER_STRING_DEFAULT);
            if (!in_array($type, [1, 2])) {//礼品类型 0:其他 1:礼品卡 2:coupon 3:实物 5:积分
                continue;
            }

            //礼品值
            $rewardName = data_get($reviewsTmp, $key . Constant::LINKER . Constant::DB_TABLE_REWARD_NAME, Constant::PARAMETER_STRING_DEFAULT);//奖励名称
            $rewardTypeValue = data_get($reviewsTmp, $key . Constant::LINKER . Constant::DB_TABLE_TYPE_VALUE, Constant::PARAMETER_STRING_DEFAULT);//奖励类型值
            $rewardValue = RewardService::getClientRewardTypeValue($storeId, $rewardName, $type, $rewardTypeValue, $currency);

            data_set($reviewsTmp, $key . Constant::LINKER . 'reward_value', $rewardValue);
        }
    ...
    {
        $remark = 'points';
        if (in_array($storeId, [1, 3]) && in_array(strtoupper($orderCountry), ['DE', 'ES', 'IT', 'FR'])) {
            $pointRemark = [
                'DE' => 'Punkte',
                'ES' => 'puntos',
                'IT' => 'punti',
     ...
     * @return array
     */
    public static function auditRemarks($requestData)
    {
        $storeId = data_get($requestData, Constant::DB_TABLE_STORE_ID, Constant::PARAMETER_INT_DEFAULT);//品牌id
        $remarksData = data_get($requestData, Constant::DB_TABLE_REMARKS, []);//备注数据

        foreach ($remarksData as $remarksItem) {

            $id = data_get($remarksItem, Constant::DB_TABLE_PRIMARY, Constant::PARAMETER_INT_DEFAULT);//主键id
            $remarks = data_get($remarksItem, Constant::DB_TABLE_REMARKS, Constant::PARAMETER_STRING_DEFAULT);//备注

            $updateData = [
                Constant::DB_TABLE_REMARKS => $remarks, //审核备注
            ];

            static::update($storeId, [Constant::DB_TABLE_PRIMARY => $id], $updateData);
        }
