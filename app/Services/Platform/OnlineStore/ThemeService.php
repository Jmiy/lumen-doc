...
class ThemeService extends BaseService {
    ...
    public static function handle($storeId, $platform, $data) {
        ...
        $_data = PlatformServiceManager::handle($platform, 'Theme', 'getThemeData', [$storeId, $platform, $data]);
    ...
    public static function handlePull($storeId, $platform, $parameters) {
        $_data = PlatformServiceManager::handle($platform, 'Theme', 'getList', $parameters);

        $response = static::getResponseData(200007, 200008, $_data);
        if ($response !== true) {
            return $response;
        }
