...
class OrderService extends BaseService {
     ...
     * @return boolean true:拉取成功 false:拉取失败
     */
    ...
    public static function isExists($storeId, $orderNo, $platform = Constant::PLATFORM_SERVICE_AMAZON) {
        ...
        return static::pullOrder($storeId, $platform, $orderNo);
    }
     ...
     * @return array 订单数据
     */
    public static function getOrderData($orderNo, $country = Constant::PARAMETER_STRING_DEFAULT, $platform = Constant::PLATFORM_SERVICE_AMAZON, $storeId = Constant::ORDER_STATUS_SHIPPED_INT, $isUseLocal = true, $warrantyId = 0) {

        $isPullSuccess = static::pullOrder($storeId, $platform, $orderNo, $isUseLocal, '', $warrantyId);
        ...
        //优先获取本系统的订单数据
        $orderData = static::getOrderDetails($storeId, $orderNo, Constant::PARAMETER_STRING_DEFAULT, $platform);
        if ($orderData && data_get($orderData, 'items', [])) {
            return Response::getDefaultResponseData(1, '', $orderData);
        }

        if($isUseLocal === true){
            return static::{__FUNCTION__}($orderNo, Constant::PARAMETER_STRING_DEFAULT, $platform, $storeId, false, $warrantyId);
        }
    ...
    public static function getOrderDataNew($orderNo, $country, $platform = Constant::PLATFORM_SERVICE_AMAZON, $storeId = Constant::ORDER_STATUS_SHIPPED_INT, $isUseLocal = true, $warrantyId = 0) {
        ...
        $orderData = static::getOrderData($orderNo, $country, $platform, $storeId, $isUseLocal, $warrantyId); //获取亚马逊订单数据
        if (data_get($orderData, Constant::RESPONSE_CODE_KEY, Constant::PARAMETER_INT_DEFAULT) == 0) {
            return $rs;
        }

        $orderData = data_get($orderData, Constant::RESPONSE_DATA_KEY, Constant::PARAMETER_ARRAY_DEFAULT);
        $orderStatus = strtolower(data_get($orderData, Constant::DB_TABLE_ORDER_STATUS, Constant::PARAMETER_STRING_DEFAULT));
        ...
        data_set($rs, Constant::RESPONSE_CODE_KEY, data_get($codeData, $orderStatus, 39001));
