...
class ActivityApplyService extends BaseService {
    ...
    public static function isCanApply($storeId, $actId, $customerId, $extId, $extType, $ip = '') {
        ...
        if ($storeId == 1) {
            ...
            if (empty($isExists)) {
                return Response::getDefaultResponseData(60002,'Please fill in the application materials and participate in the event.');
            }
        } else {
            $isExists = ActivityApplyInfoService::exists($storeId, $actId, $customerId);
            if (!empty($isExists)) {
                return Response::getDefaultResponseData(60005);
            }
        }
        ...
        if ($applyData) {//如果有 审核中\审核通过\其他 的申请，就不可以继续申请 并根据审核状态进行提示
            switch ($applyData->audit_status) {
                ...
                case 0://审核中
                case 3://其他
                    return Response::getDefaultResponseData(60003);
                    break;
    ...
    public static function isCanApplyFree($storeId, $actId, $customerId, $extId, $extType, $account, $applyCountry) {
        ...
        //判断申请用户的国家是否在产品国家列表中
        $countryJudgeConfig = ActivityService::getActivityConfigData($storeId, $actId, 'apply', 'country_judge');
        $isCountryJudge = data_get($countryJudgeConfig, "apply_country_judge.value");
        if ($isCountryJudge) {
            $activityProductItemCountries = ActivityProductItemService::getModel($storeId)
                ->select(Constant::DB_TABLE_COUNTRY)
                ->where(Constant::DB_TABLE_PRODUCT_ID, $extId)
                ->get();
            if (empty($activityProductItemCountries)) {
                return Response::getDefaultResponseData(60006);
            }
            $applyCountry = strtolower($applyCountry);
            foreach ($activityProductItemCountries as $activityProductItemCountry) {
                $productItemCountry = strtolower(data_get($activityProductItemCountry, Constant::DB_TABLE_COUNTRY));
                if ($productItemCountry == 'all' || $productItemCountry == $applyCountry) {
                    return Response::getDefaultResponseData(1);
                }
            }

            return Response::getDefaultResponseData(60013);
        }

        //判断申请的产品库存
        if ($productData[Constant::DB_TABLE_QTY_APPLY] >= $productData['qty']) {//如果产品已经被申请完，就提示用户
            //发送库存不足邮件
            return Response::getDefaultResponseData(60007,'Aplications are ran out, please apply other products.');
        }
...
//        }
    ...
    public static function isCanApplyAccessories($storeId, $actId, $customerId, array $extIds, $extType, $ip) {
        ...
        if (count($products) < count($extIds)) {
            return Response::getDefaultResponseData(60014);
        }
