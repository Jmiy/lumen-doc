...
class InviteCodeService extends BaseService {
     ...
     * @param  array  $customerSelect
     * @return array|mixed
     */
    public static function getInviteInfo(int $storeID, string $inviteCode, array $customerSelect=[])
    {
        $select = ['customer_id'];
        $customerInfoSelect = !empty($customerSelect) ? $customerSelect : ['first_name', 'last_name'];

        $where = ['invite_code' => $inviteCode];
        $exePlan = FunctionHelper::getExePlan('default_connection_'.$storeID, null,
            'InviteCode', '', $select, $where);

        $itemHandleDataCallback = [
            'account' => function ($item) use ($storeID, $customerInfoSelect) {
                $accountInfo = CustomerInfo::where([
                    Constant::DB_TABLE_CUSTOMER_PRIMARY => data_get($item, 'customer_id'),
                    Constant::DB_TABLE_STORE_ID => $storeID
                ])->select($customerInfoSelect)->first();
                return $accountInfo ?? [];
            }
        ];
        ...
        $info = FunctionHelper::getResponseData(null, $dbExecutionPlan);
        if ($info && !empty($info['account'])) {
            return $info['account'];
        } else {
            return [];
        }
