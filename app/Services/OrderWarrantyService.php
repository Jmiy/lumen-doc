...
//        return $retult;
    ...
    public static function getOrderEmailData($storeId = 0, $orderId = 0, $actId = 0) {
        ...
        $handleData = [
            ...
            Constant::DB_TABLE_ORDER_TIME => FunctionHelper::getExePlanHandleData('orderStatus{or}' . Constant::DB_TABLE_ORDER_TIME, $default, $data, Constant::DB_EXECUTION_PLAN_DATATYPE_DATETIME, data_get($orderConfig, Constant::CONFIG_KEY_WARRANTY_DATE_FORMAT, 'Y-m-d H:i'), $time, $glue, $isAllowEmpty, $callback, $only), //订单时间
        ];
...
//        }
    ...
    public static function handleCreditAndExp($storeId, $orderWarrantyId, $actId = 0, $extData = []) {
        ...
        if ($value != 0) {
            ...
            //加积分
            $supportCredit = DictStoreService::getByTypeAndKey($storeId, Constant::ORDER_BIND, 'support_credit', true, true);
            if ($supportCredit) {//如果延保添加积分，就添加积分
                $isExists = CreditService::existsOrFirst($storeId, '', $creditWhere);
     ...
     * @return boolean
     */
    public static function editInsurance($storeId, $where, $data) {
    ...
    /**
     * 获取 使用 $version 版本规则的延保品牌
     * @param $storeId
     * @param string $version
     * @return int[]
     */
    public static function getWarranyStore($storeId, $version = 'v2')
    {
        $extWhere = [
            Constant::DICT => [
                Constant::DB_TABLE_DICT_KEY => $version,
            ],
            Constant::DICT_STORE => [
            ],
        ];
        $config = OrderReviewService::getMergeConfig($storeId, Constant::RESPONSE_WARRANTY, $extWhere);
        $store = data_get($config, $version);

        return $store !== null ? explode(',', $store) : StoreService::getStoreIds()->pluck('id')->all();
    }
}
