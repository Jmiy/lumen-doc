...
//                ->later($when, $message);
     ...
     * @return string
     */
    public static function getActivateSum($storeId, $account, $type){

        if (empty($storeId) || empty($account) || empty($type)) {
            return false;
        }
        ...
        $res = static::getModel($storeId)
            ->select(Constant::DB_TABLE_PRIMARY)
            ->where('store_id', '=', $storeId)
            ->where('to_email', '=', $account)
            ->where('type', '=', $type)
            ->where('ctime', '>', $startTime)
            ->where('ctime', '<', $endTime)
            ->where('status', '=', 1)
            ->get();
        $res = !empty($res->toArray()) ?  $res->toArray() : [];
