...
class RewardService extends BaseService {
    ...
    public static function add($storeId, $requestData) {
        ...
        if ($type == 1 && data_get($giftRecords,Constant::RESPONSE_CODE_KEY) !== null) {//如果礼品卡值未填写，就提示用户
            return $giftRecords;
        }
    ...
    public static function buildRewardAsin($data) {
        ...
        foreach ($data as $item) {
            $asin = data_get($item, Constant::DB_TABLE_ASIN);
            $country = data_get($item, Constant::DB_TABLE_COUNTRY);
            $code = data_get($item, Constant::RESPONSE_CODE_KEY);
            $codeType = data_get($item, Constant::DB_TABLE_CODE_TYPE);//code类型：1优惠券,2礼品卡
            ...
            if (!isset($asinRecords[$key])) {
                ...
                if ($codeType == 2) {//如果是礼品卡，就设置类型值
                    $asinRecords[$key][Constant::DB_TABLE_TYPE_VALUE] = $code;
                }
            }
        }
    ...
    public static function getAsinCode($requestData, $group = 'reward') {
        ...
        if ($type == 1) {
            ...
            if(data_get($giftRecords,Constant::RESPONSE_CODE_KEY) === null){
    ...
    public static function addCodes($storeId, $rewardId, $records) {
        ...
        foreach ($records as $record) {
            ...
            $dbRet = CouponService::updateOrCreate($storeId, $where, $data);
    ...
    public static function convertToTableData($excelData, $group = 'reward', $useType = 1) {
        ...
        foreach ($excelData as $k => $row) {
            ...
            data_set($tableData, $k . Constant::LINKER . Constant::DB_TABLE_CODE_TYPE, 1);//code类型：1优惠券,2礼品卡
    ...
    public static function giftConvertToTableData($excelData, $name, $group = 'reward', $useType = 2) {
        ...
        foreach ($excelData as $k => $row) {
            ...
            $country = data_get($row, data_get($header, Constant::DB_TABLE_COUNTRY), Constant::PARAMETER_STRING_DEFAULT);
            $asin = data_get($row, data_get($header, Constant::DB_TABLE_ASIN), Constant::PARAMETER_STRING_DEFAULT);
            $code = data_get($row, data_get($header, Constant::DB_TABLE_NAME), Constant::PARAMETER_STRING_DEFAULT);

            if (empty($country)) {
                return Response::getDefaultResponseData(2, '站点未填写');
            }

            if (empty($asin)) {
                return Response::getDefaultResponseData(3, 'Asin未填写');
            }

            if (empty($code)) {
                return Response::getDefaultResponseData(4, '礼品卡值未填写');
            }

            $codeNumber = FunctionHelper::getNumber($code);
            if ($codeNumber === null) {
                return Response::getDefaultResponseData(5, '礼品卡值非数值/数值过大，请重新输入');
            }

            if ($codeNumber <= 0) {
                return Response::getDefaultResponseData(6, '礼品卡数值不得为0，请重新输入');
            }

            if ($codeNumber < 1 || $codeNumber > 999) {
                return Response::getDefaultResponseData(7, '礼品卡值非数值/数值过大，请重新输入');
            }
            ...
            data_set($tableData, $k . Constant::LINKER . Constant::DB_TABLE_CODE_TYPE, 2);//code类型：1优惠券,2礼品卡
            data_set($tableData, $k . '.use_type', $useType);
        }
    ...
    public static function getList($params, $toArray = true, $isPage = true, $select = [], $isRaw = false, $isGetQuery = false, $isOnlyGetCount = false) {
        ...
        if (data_get($params, Constant::DB_EXECUTION_PLAN_IS_ONLY_GET_PRIMARY, false)) {//如果仅仅获取主键id，就不需要处理数据，不关联
            ...
            $itemHandleDataCallback = [
                Constant::RESPONSE_CODE_KEY => function ($item) use ($storeId) {//coupon code

                    $type = data_get($item, Constant::DB_TABLE_TYPE); //礼品类型: 0:其他 1:礼品卡 2:coupon 3:实物 5:积分
                    if ($type != 2) {//如果不是coupon,就直接返回空
                        return '';
                    }
                    ...
                    return CouponService::getModel($storeId)->buildWhere($where)->limit(10)->pluck(Constant::RESPONSE_CODE_KEY)->implode(',');
                },
                Constant::DB_TABLE_TYPE_VALUE => function ($item) use ($storeId, $data, $time, $isAllowEmpty, $callback, $only) {//类型值
                    $type = data_get($item, Constant::DB_TABLE_TYPE); //礼品类型: 0:其他 1:礼品卡 2:coupon 3:实物 5:积分

                    if ($type != 1) {//如果不是礼品卡,就直接返回rewards表中 Constant::DB_TABLE_TYPE_VALUE 字段对应的值
                        return data_get($item, Constant::DB_TABLE_TYPE_VALUE);
                    }

                    return FunctionHelper::handleData(
                        $item,
                        FunctionHelper::getExePlanHandleData(
                            'asins.*.' . Constant::DB_TABLE_TYPE_VALUE,
                            Constant::PARAMETER_STRING_DEFAULT,
                            $data,
                            Constant::DB_EXECUTION_PLAN_DATATYPE_STRING,
                            Constant::DB_EXECUTION_PLAN_DATATYPE_STRING,
                            $time,
                            ',',
                            $isAllowEmpty,
        ...
        if ($data) {
            ...
            foreach ($typeData as $type) {
                ...
                if ($reward && in_array($type, [1, 2])) {
                    ...
                    if ($type == 1) {//如果是礼品卡，要根据 asin+国家+ext_id+ext_type
                        $couponExtWhere = [
                            'collapseWhere' => [
                                Constant::DB_TABLE_ASIN => $asin,
                                Constant::DB_TABLE_COUNTRY => $country,
                    ...
                    $couponData = CouponService::getRewardCoupon($storeId, $rewardId, static::getModelAlias(), 0, $name, '', $couponExtWhere);
                    if ($couponData) {
                        break;
                    } else {
                        $reward = null;
    ...
    public static function handleOrderReviewReward($storeId, $customerId = 0, $orderReviewId = 0, $orderno = '', $account = '') {
        ...
        $country = data_get($reward, Constant::DB_TABLE_PRODUCT_COUNTRY, Constant::PARAMETER_STRING_DEFAULT); //订单国家
        ...
        try {
            ...
            switch ($type) {
                ...
                case 2://coupon
                    ...
                    $couponData = CouponService::getRewardCoupon($storeId, $rewardId, static::getModelAlias(), $customerId, $name, $account, $couponExtWhere);
    ...
    public static function handleOrderReviewRewardV2($storeId, $customerId, $orderReview, $account, $orderData) {
        ...
        $country = data_get($orderReview, Constant::DB_TABLE_COUNTRY, Constant::PARAMETER_STRING_DEFAULT);//订单国家
     ...
     * @param string $rewardTypeValue 奖励类型值
     * @param string $currency 货币
     * @return array|mixed|string
     */
    public static function getClientRewardTypeValue($storeId = 0, $rewardName = '', $rewardType = 0, $rewardTypeValue = '', $currency = '')
    {

        switch (true) {
            case (in_array($rewardType, [2]))://coupon
                //从礼品名称中获取折扣
                $rewardTypeValue = FunctionHelper::getNumber($rewardName) ?? Constant::PARAMETER_INT_DEFAULT;

                break;

            case (in_array($rewardType, [1]))://礼品卡
                //从礼品值中获取礼品卡金额
                $rewardTypeValue = FunctionHelper::getNumber($rewardTypeValue);
                $rewardTypeValue = $rewardTypeValue ?? (FunctionHelper::getNumber($rewardName) ?? Constant::PARAMETER_INT_DEFAULT);

                if($currency){
                    $currencyData = static::getConfig($storeId, 'currency');
                    $rewardTypeValue = data_get($currencyData, $currency, '') . '' . $rewardTypeValue;
                }
