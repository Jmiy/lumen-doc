            ...
            if (!empty($productCountry)) {
                ...
                $freeTestCustomizeWhere[] = [
                    ...
                    Constant::PARAMETERS_KEY => function ($query) use ($productCountries, $storeId) {
                        $query->whereRaw("EXISTS(select 1 from `ptxcrm`.`crm_metafields` WHERE store_id = $storeId and owner_resource = 'ActivityProduct' and owner_id = `crm_activity_products`.`id` and namespace = 'free_testing' and `key` = 'country' and status = 1 and `value` IN ($productCountries)) OR crm_activity_products.country IN ($productCountries)");
                    },
                ];
            }
...
//        }
    ...
    public static function getUniversaData($storeId, $actId, $country, $type, $page, $pageSize, $extData = []) {
        ...
        if (!empty($avtivityData)){
            $expire_status = time() < strtotime($avtivityData['end_at']) ? 1 : 0 ;
            $avtivityData['expire_status'] = $expire_status;
        }
    ...
    public static function getFreeTestingList($params, $toArray = true, $isPage = true, $select = [], $isRaw = false, $isGetQuery = false, $isOnlyGetCount = false) {
        ...
        //查询条件
        data_set($params, 'tableAlias.actProduct', 'activity_products.');
        ...
        $itemHandleDataCallback = [
            ...
            'export_sku' => function($item) {
                return data_get($item, Constant::DB_TABLE_SHOP_SKU, data_get($item, Constant::DB_TABLE_SKU, ''));
            }
        ];
