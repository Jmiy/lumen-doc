...
class ActivityWinningService extends BaseService {
     ...
     * @param int|null $activityWinningId 中奖流水 主键 id
     * @return type
     */
    public static function getItem($storeId, $actId = 0, $prizeId = 0, $customerId = 0, $prizeItemId = 0, $activityWinningId = null) {
        ...
        $order = [['w.' . Constant::DB_TABLE_UPDATED_AT, Constant::DB_EXECUTION_PLAN_ORDER_DESC]];

        if ($activityWinningId !== null) {
            $where = [
                ('w.' . Constant::DB_TABLE_PRIMARY) => $activityWinningId,
            ];

            $order = [];
        }
        $dbExecutionPlan = static::getWinningPrizeDbExecutionPlan($storeId, $where, $order);
     ...
     * @param int $storeId 商城id
     * @param int $actId 活动id
     * @param int $isNeedParticipationAward 中奖排行榜是否需要安慰奖 1:需要  0:不需要 默认:0
     * @return boolean true:初始化成功
     */
    public static function initRank($storeId = 0, $actId = 0, $isNeedParticipationAward = 0) {
        $zsetKey = static::getRankKey($storeId, $actId);
        ...
        if ($isNeedParticipationAward == 0) {//如果不需要安慰奖，就只获取非安慰奖数据
            $publicData[Constant::DB_TABLE_IS_PARTICIPATION_AWARD] = 0;//排行榜 默认 只获取非参与奖的奖项
        }
