    ...
    public static function reg($storeId, $account, $storeCustomerId = 0, $createdAt = '', $source = 1, $country = '', $firstName = '', $lastName = '', $gender = 0, $brithday = '', $orderno = '', $lastlogin = '', $ip = '', $data = []) {
        ...
        $rs = Cache::lock('reg:' . $storeId . ':' . $account)->get(function () use ($storeId, $account, $storeCustomerId, $createdAt, $source, $country, $firstName, $lastName, $gender, $brithday, $orderno, $lastlogin, $ip, $data, $logParameters) {
            ...
            if ($isExists) {
                ...
                if ($storeId == 14){
                    data_set($_data, Constant::RESPONSE_DATA, [
                        Constant::RESPONSE_CODE_KEY => 10030,
                        Constant::RESPONSE_MSG_KEY => 'customer  exists',
                        Constant::RESPONSE_DATA_KEY => [],
                    ]);
                }
            ...
            case 'register':
                ...
                if ($messageData) {
                    ...
                    if (false !== strpos($message, 'Email has already been taken')) {//如果 $platform 平台存在 $account 账号，就提示用户
                        ...
                        $rs[Constant::RESPONSE_MSG_KEY] = '';
                        if ($storeId == 14){
                            $rs[Constant::RESPONSE_CODE_KEY] = 10030;
                        }
                    }
...
//        ResponseLogService::update($storeId, [Constant::DB_TABLE_STORE_ID => $storeId, Constant::DB_TABLE_ACCOUNT], [Constant::DB_TABLE_ACCOUNT => $newAccount]);
     ...
     * @return bool
     */
    public static function readShopfiyAppAccount($storeId, $filePath)
    {
        ...
        while (!feof($file)) {
            ...
            $data = array_map(function ($vale) use ($i) {
                return $i == 0 ? strtoupper(trim($vale)) : trim($vale);
            }, $data);


            if ($i == 0) {
                $headData = array_flip($data);
                $i++;
                continue;
            }

            $_data = [];
            foreach ($headData as $key => $index) {
                $_data[$key] = data_get($data, $index, '');
            }

            //使用消息队列处理数据，处理好数据以后再使用消息队列将处理好的数据写入数据库
            $service = static::getNamespaceClass();
            $method = 'handleShopfiyAppAccount';

            //设置 消息队列 的任务（任务：将处理好的数据写入数据库）
            $queueData = FunctionHelper::getJobData($service, 'importShopfiyAppAccount', [$storeId]);

            $i++;
            $parameters = [$storeId, $_data, $queueData];
    ...
    /**
     * 格式化 Shopfiy App Account 的数据
     * @param $storeId 商城id
     * @param $data Shopfiy App Account 的数据
     * @param null $queueData 消息队列 的任务（任务：将处理好的数据写入数据库）
     * @return array
     */
    public static function handleShopfiyAppAccount($storeId, $data, $queueData = null)
    {
        //获取 国家  州  城市  数据
        $location = data_get($data, 'LOCATION', '');//LOCATION  数据规则：(城市  国家) 或者 (城市  州)
        $ip = data_get($data, 'IP', '');
        $address = [];
        $ipCountry = '';//ip 国家
        $country = '';
        if (!empty($location)) {
            $location = explode('  ', data_get($data, 'LOCATION', ''));//使用 '  '  拆开
            $count = count($location);

            $countryOrRegion = end($location);//获取 国家 或者 州

            //通过国家表查询 $countryOrRegion 对应的国家
            $countryData = CountryService::existsOrFirst($storeId, '', ['country_name' => $countryOrRegion], true, ['country_code']);
            $_countryOrRegion = data_get($countryData, 'country_code');
            if ($_countryOrRegion === null && $ip) {//如果通过国家表查询不到国家，就通过ip获取国家
                $ipCountry = FunctionHelper::getCountry($ip, (strtoupper($countryOrRegion) == strtoupper('Hong Kong') ? 'HK' : ''));
            }

            if ($count == 1) {//LOCATION  里面只有 国家，账号地址只记录国家即可
                $country = $_countryOrRegion ?? $ipCountry;
                $address = [
                    Constant::DB_TABLE_COUNTRY => $country,
                ];
            } else {//LOCATION  里面有 城市  国家 或者 城市  州
                if ($_countryOrRegion === null) {//如果通过国家表查询 $countryOrRegion 对应的国家不存在

                    if (mb_strlen($countryOrRegion) == 2) {//如果是 城市  州,国家默认使用 US，账号地址只记录国家 州 城市
                        $country = 'US';
                        $address = [
                            Constant::DB_TABLE_COUNTRY => $country,// 国家
                            Constant::DB_TABLE_REGION => $countryOrRegion,// 州
                            Constant::DB_TABLE_CITY => reset($location),// 城市
                        ];
                    } else {//如果是 城市  国家,就使用通过ip查询到的国家，账号地址只记录国家和城市
                        $country = $ipCountry;
                        $address = [
                            Constant::DB_TABLE_COUNTRY => $country,// 国家
                            Constant::DB_TABLE_CITY => reset($location),// 城市
                ...
                } else {//如果通过国家表查询 $countryOrRegion 对应的国家存在，就使用查询到的国家，账号地址只记录国家 城市
                    $country = $_countryOrRegion;
                    $address = [
                        Constant::DB_TABLE_COUNTRY => $_countryOrRegion,
                        Constant::DB_TABLE_CITY => reset($location),
                    ];
                }
            }

            if(empty($country)){//如果国家为空，就不记录账号地址
                $address = [];
            }

        }

        $createdAt = data_get($data, 'DATE');
        if($createdAt !== null){
        ...
        $item = [
            Constant::DB_TABLE_STORE_ID => $storeId,
            Constant::DB_TABLE_ACCOUNT => data_get($data, 'EMAIL', ''),
            Constant::DB_TABLE_FIRST_NAME => data_get($data, 'FIRST_NAME', ''),
            Constant::DB_TABLE_LAST_NAME => data_get($data, 'LAST_NAME', ''),
            Constant::DB_TABLE_COUNTRY => $country,
            Constant::DB_TABLE_OLD_CREATED_AT => $createdAt ? FunctionHelper::handleDatetime($createdAt, 'Y-m-d H:i:s', 'date') : Carbon::now()->toDateTimeString(),
            Constant::DB_TABLE_SOURCE => 2,//注册方式：1:自然注册 2:常规活动 3:大型活动 4:非官方页面 5:后台同步 6:定时任务同步 7:程序自动注册
            Constant::DB_TABLE_IP => data_get($data, 'IP', ''),
            Constant::DB_TABLE_ADDRESS => $address,
        ];

        if ($queueData !== null) {
            data_set($queueData, Constant::PARAMETERS_KEY . Constant::LINKER . '1', [$item]);
            FunctionHelper::pushQueue($queueData, null, '{data-import}'); //把任务加入消息队列
        }
     ...
     * 规则：
     * 1：中国的账号不同步
     * 2：包含有 '@qq.com', '@163.com', '@patazon.net', '@chacuo.net' 的账号不同步
     * @param int $storeId 官网id
     * @param array $data 账号数据
     * @return bool
     */
    public static function importShopfiyAppAccount($storeId, $data)
    {
        ...
        foreach ($data as $row) {
            ...
            $country = $row[Constant::DB_TABLE_COUNTRY] ?? '';
            $account = $row[Constant::DB_TABLE_ACCOUNT] ?? '';
            if (in_array(strtoupper($country), ['CN', 'HK']) || empty($account) || !(static::isEffectiveAccount($account))) {//如果是中国或者香港的账号 或者 $account不是有效账号，就不同步到会员系统
                continue;
            }
            ...
            $lastlogin = $row[Constant::DB_TABLE_LASTLOGIN] ?? '';
            $ip = $row[Constant::DB_TABLE_IP] ?? '';
            data_set($row, Constant::DB_TABLE_ADDRESS, CustomerAddressService::getNotExistsUpdateData(data_get($row, Constant::DB_TABLE_ADDRESS, [])));

            $exists = static::customerExists($storeId, 0, $account, 0, true, [Constant::DB_TABLE_CUSTOMER_PRIMARY, Constant::DB_TABLE_SOURCE]); //获取 $account 对应的有效账号
            if ($exists) {//如果 $account 是有效的账号，就更新账号数据
                $customerId = data_get($exists, Constant::DB_TABLE_CUSTOMER_PRIMARY, -1);
                $srcSource = data_get($exists, Constant::DB_TABLE_SOURCE, -1);

                $where = [Constant::DB_TABLE_CUSTOMER_PRIMARY => $customerId];
                if (in_array($srcSource, [5, 6]) && $ip && $country) {//如果 ip和国家 同时存在 并且 来源为shopify同步的，就把来源修改为：常规活动
                    static::update($storeId, $where, [Constant::DB_TABLE_SOURCE => $source]);
                }

                $select = [Constant::DB_TABLE_FIRST_NAME, Constant::DB_TABLE_LAST_NAME, Constant::DB_TABLE_IP, Constant::DB_TABLE_COUNTRY];
                $_customerInfo = CustomerInfoService::existsOrFirst($storeId, '', $where, true, $select); //获取 $account 对应的有效账号
                $replacePairs = [
                    "'" => "\'",
                    "?" => "\?",
                ];
                ...
                if (empty(data_get($_customerInfo, Constant::DB_TABLE_FIRST_NAME))) {//如果 first name 为空，就更新first name
                    $updateData[Constant::DB_TABLE_FIRST_NAME] = $firstName;
                }

                if (empty(data_get($_customerInfo, Constant::DB_TABLE_LAST_NAME))) {//如果 last name 为空，就更新last name
                    $updateData[Constant::DB_TABLE_LAST_NAME] = $lastName;
                }

                if (empty(data_get($_customerInfo, Constant::DB_TABLE_IP))) {//如果 ip 为空，就更新ip
                    $updateData[Constant::DB_TABLE_IP] = $ip;
                }

                if (empty(data_get($_customerInfo, Constant::DB_TABLE_COUNTRY))) {//如果 country 为空，就更新country
                    $updateData[Constant::DB_TABLE_COUNTRY] = $country;
                }
            ...
            try {
                $customerData = static::reg($storeId, $account, 0, $createdAt, $source, $country, $firstName, $lastName, 0, '', '', $lastlogin, $ip, $row);
                $customerId = $customerData[Constant::CUSTOMER_ID] ?? 0;
                if ($customerId) {
                    static::regInit($storeId, $customerId);
                }
