...
class FunctionHelper {
    ...
    public static function getClientIP($ip = null) {
        ...
        return app('request')->getClientIp() ?? '127.0.0.0';
    }
...
//            }
        ...
        //邀请注册活动-优化获取IP组件（针对GB转化为UK）Jmiy 2021-05-05 14:50  update : http://jira.patozon.net/browse/GW-1052
        if($country && strtoupper($country) == 'GB'){//如果国家不为空并且是 GB，就转化为UK
            $country = 'UK';
        }
    ...
    public static function setTimezone($storeId, $timezone = '', $dbTimezone = '', $appEnv = null)
    {
        ...
        if (empty($dbTimezone) && $storeId) {
            $dbTimezone = data_get($storeTimezone, 'db_timezone', $dbTimezone);
        }
        ...
        if ($dbTimezone) {
            ...
            foreach ($databaseConnections as $connection => $value) {
                if (isset($value['timezone'])) {
                    config(['database.connections.' . $connection . '.timezone' => $dbTimezone]);
                }
            }
        }
    ...
    /**
     * lucky number活动新增的字符串脱敏规则 muc**@example.com
     *
     * @param  string  $email 需要脱敏的邮箱字符串
     * @param  string  $symbol  脱敏替代字符
     * @param  int  $symbolNum 脱敏替代字符数量
     * @param  int  $showLength 展示邮箱前缀的总长度
     * @return string
     */
    public static function handleAccountEmail($email="", $symbolNum=4, $symbol="*", $showLength=10)
    {
        if (empty($email)) {
            return $email;
        }
        // 暂时全部使用邮箱的规则
        $suffixPos = strrpos($email, '@');
        $originalPrefixStr = substr($email, 0, $suffixPos);
        $suffixStr = substr($email, $suffixPos);

        $length = Str::length($originalPrefixStr);

        if ($length <= $symbolNum) {
            $symbolNum = $length;
        }
        $showPrefix = substr($email, 0, ($length - $symbolNum));
        // 当整体邮箱前缀长度超过了最大限定值 需要截断
        if ($showLength <= Str::length($showPrefix)) {
            $showPrefix = substr($showPrefix, 0, ($showLength - $symbolNum));
        }

        $prefixStr = $showPrefix.str_repeat($symbol, $symbolNum);
     ...
     * @param  int  $count
     * @param  null  $num 传值
     * @param  null  $amendSymbol 替代符号
     * @return false|string
     */
    public static function getRandNum($count=3, $num=null, $amendSymbol=null)
    {
        if (!is_null($num)) {
            $nCount = strlen("".$num);
            if ($nCount < $count) {
                $amendSymbol = $amendSymbol ?? "0";
                return str_repeat($amendSymbol, ($count - $nCount)) . $num;
            } else{
                return substr($num . "", 0, $count);
            }

        } else {
            // $str = substr(Str::random(16),0, mt_rand(1, 16));
            // $str = bin2hex($str);
            // preg_match_all('/[0-9]/', $str, $match);
            // return substr(implode('', $match[0]), 0, $count);
            $str = (string)lcg_value();
            return substr($str,2, $count);
        }
    }
    ...
    public static function handleDatetime($data, $format = null, $attributes = null, $timezone = null) {
        ...
        $timestamp = strtotime($timeData);
        /**
         * m / d / y或d-m-y格式的日期通过查看来消除歧义   各个组件之间的分隔符：如果分隔符是a   斜线（/），然后是美国m / d / y; 而如果   separator是短划线（ - ）或点（。），然后是欧洲d-m-y格式
         */
//        $delimiterData = [
//            '/' => '-',
//            '-' => '/',
...
//
//            $timestamp = strtotime(str_replace($key, $value, $timeData));
//
//            dump(str_replace($key, $value, $timeData),$timestamp);
//
//            if($timestamp !== false){
//                break;
//            }
...
//        }
    ...
    public static function pushQueue($job, $data = '', $queue = null)
    {
        ...
        try {
            $connectionName = data_get($job, 'queueConnectionName');
            $queue = $queue !== null ? $queue : data_get($job, 'queue');

            if (is_object($job) || is_string($job)) {

                if ($connectionName) {
                    return Queue::connection($connectionName)->push($job, $data, $queue);
                }
            ...
            if ($connectionName) {
                return Queue::connection($connectionName)->push(PublicJob::class, $job, $queue);
            }

            return Queue::push(PublicJob::class, $job, $queue);
            //return Queue::push(new PublicJob($job), $data, $queue);
        } catch (\Exception $exc) {
    ...
    public static function laterQueue($time, $job, $data = '', $queue = null) {
        ...
        try {
            ...
            if (is_object($job) || is_string($job)) {
                ...
                if ($connectionName) {
                    return Queue::connection($connectionName)->later($time, $job, $data, $queue);
                }

                return Queue::later($time, $job, $data, $queue);
            }

            if ($connectionName) {
                return Queue::connection($connectionName)->later($time, PublicJob::class, $job, $queue);
            }
            return Queue::later($time, PublicJob::class, $job, $queue);
...
//        return \App\Services\UniqueIdService::getUniqueId(...$parameters);
    ...
    public static function checkOrderNo($orderNo) {
        ...
        return preg_match('/^([\d]{3}-[\d]{7}-[\d]{7})$/', $orderNo) || preg_match('/^(S[\d]{2}-[\d]{7}-[\d]{7})$/', $orderNo) || preg_match('/^(mpow[\d]+)$/', $orderNo);
    }

    public static function setLocale($country) {
        app('translator')->setLocale($country);
    }
    ...
    public static function isSwooleRun() {
