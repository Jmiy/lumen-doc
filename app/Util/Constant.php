...
class Constant {
    ...
    const SYSTEM_REMARKS = 'system_remarks';//系统备注
    const SHOP_NAME = 'shop_name';//店铺名称
    const ROUTE_ID = 'route_id';//路由id

    const CASH_BACK_TYPE = 'cash_back_type'; //返现类型
    const CASH_BACK_TIME = 'cash_back_time'; //返现时间
    const APPLY_SKU = 'apply_sku'; //申请测评产品
    const REIMBURSE_TYPE = 'reimburse_type'; //报销类型
    const REIMBURSE_AMOUNT = 'reimburse_amount'; //报销金额
    const APPROVED_NUM = 'approved_num'; //审核通过数量
    const TAG_ID = 'tag_id'; //测评用户标签id
    const TAG_NAME = 'tag_name'; //测评用户标签名称
    const REASON = 'reason'; //黑名单原因
    const CASH_BACK_ACCOUNT = 'cash_back_account'; //返现账号

    const PLATFORM_SERVICE_PATOZON_XC = 'Patozon_XC'; //销参

    const ADD_TOTAL_CACHE = 'lotteryStatistics';//累计增加次数缓存key
    const ACT_HOUR_LIMIT_KEY = 'hour_limit';//小时限制
    const ADD_TOTAL = 'addTotal';//累计增加次数
    const DB_TABLE_ADD_TOTAL = 'add_total';//累计增加次数
    const REMAINING_TOTAL = 'remaining_total';//剩余次数
    const INIT = 'init';//初始化次数
    const INIT_DB = 'initDb';//初始化次数
    const USED_TOTAL = 'lotteryUsedNum';//已使用总次数
    const ACT_FORM = 'act_form';//活动方式
    const TTL = 'ttl';//缓存到期时间key
    const TAG = 'tag';//缓存tag key
    const TIMESTAMP = 'timestamp';//时间戳 key
    const LIMIT_KEY = 'limitKey';//限制级别：'limit':整个活动期间 'month_limit':按月 'week_limit':按周 'day_limit':按天 'hour_limit':按小时 默认：按天
    const ACT_CONFIG = 'actConfig';//活动配置
    const ADD_NUMS = 'add_nums';//活动添加次数key
    const INVITE_DATA = 'inviteData';//邀请数据key
    const EVENT_DATA = 'eventData';//事件数据key
    const DECREMENT_NUM = 'decrement_num';//递减次数key
    const PLAY = 'play';//玩游戏key

    const SHOW_GUESS_NUM = 'show_guess_num';//猜测的数字 如:002 301
    const GUESS_NUM = 'guess_num';//猜测的数字 如:2 301
    const BASE = 'base';//基本key
    const PROVIDER = 'provider';//服务提供者key
    const PROVIDER_DEFAULT = 'Service';//默认服务提供者key
    const EVENTS = 'events';//事件
    const LISTENERS = 'listeners';//时间监听者

    const NUM = 'num';//数量
    const SHOW_NUM = 'show_num';//数量
    const DATE = 'date';//时间戳(精确到天 单位:秒)
    const PRIZE_NUM = 'prize_num';//开奖数量
    const RAND_NUM = 'rand_num';//随机数位数
    const IS_WIN = 'is_win';//是否中奖
    const DIFF_NUM = 'diff_num';//差值
    const LUCKY_NUM_ID = 'lucky_num_id';//开奖批次id
    const EVENT_UPDATE_NUM = 'update_num';//事件 次数更新事件
    const PRIZE_TYPE = 'prize_type';//奖品类型
    const WIN_LOG_ID = 'win_log_id';//中奖流水id
    const MIN_PRIZE_NUM = 'min_prize_num';//最小中奖数

    const ACTION_SHARE = 'share';//用户行为:分享tw
    const ACTION_SHARE_FB = 'share_fb';//用户行为:分享FB
    const ACTION_SHARE_TW = 'share_tw';//用户行为:分享tw
    const PRIZE_DATA = 'prizeData';//奖品数据
    const ACT_CONFIG_TYPE_WINNING = 'winning';//抽奖活动配置基础KEY
    const CONTRACT = 'contract';//服务 contract
    const CONTRACT_DEFAULT = 'Service';//默认服务 contract
    const IS_START = 'is_start';//是否开始
    const IS_END = 'is_end';//是否结束
}
